...
本文档指导你从源码构建 midscene 定制化版本，并分发给其他用户一键安装。
...
> 打包脚本会逐个重新构建每个包（绕过 nx 缓存），并验证关键导出是否完整。如果版本号变了，安装脚本里的 `VERSION` 变量也要同步修改（在 `scripts/install-bundle.sh` 第 14 行）。
...
脚本自动完成：卸旧版 → 安装依赖 → 全局注册 `midscene` 命令。
...
2. **逐个重新构建**每个包（绕过 nx 缓存，确保产物最新）
3. **验证关键导出**（如 `@midscene/computer` 的 `agentForComputer`），缺失则 clean rebuild 或 patch
4. 对每个包执行 `pnpm pack`，输出到 `tgz-output/` 目录

### install-bundle.sh（安装脚本）

1. 卸载旧版本
2. 创建 `~/.midscene-bundle` 目录，生成 `package.json`：
...
4. 全局安装 `@midscene/cli`，注册 `midscene` 命令
5. 验证安装结果
...
- 用 `npm install -g` 直接安装多个 tgz（依赖关系会乱）
- 把 tgz 发到公共 npm registry（包名还是 `@midscene/*`，会冲突）
- 把 tgz 提交进 git 仓库（应由 CI 生成）
- 删除 `~/.midscene-bundle` 目录（全局 midscene 依赖它）
